...
class _IconColorPickerScreenState extends State<EditCategory> {
  ...
  void initState() {
    super.initState();
    _fetchTaskData(); // เรียกใช้ฟังก์ชันดึงข้อมูลเก่า
  }
  ...
  Future<void> _fetchTaskData() async {
    try {
      ...
      // ดึงข้อมูลจาก Firestore ตาม categoryId
      ...
      if (categorySnapshot.exists) {
        ...
        setState(() {
          categoryController.text =
              categoryData['name'] ?? ''; // แสดงชื่อหมวดหมู่
          selectedColor = Color(categoryData['color']); // แสดงสี
          selectedIcon = IconData(categoryData['icon'],
              fontFamily: 'MaterialIcons'); // แสดงไอคอน
        });
      }
    } catch (e) {
  ...
  Future<void> _updateData() async {
    ...
    try {
      await FirebaseFirestore.instance
          ...
          .collection('category')
          .doc(widget
              .categoryId) // ใช้ categoryId เพื่ออัปเดตข้อมูลที่ต้องการแก้ไข
          .update({
        'name': categoryName,
                ...
                itemBuilder: (context, index) {
                  ...
                  return GestureDetector(
                    ...
                    child: Icon(
                      icon,
                      color:
                          selectedIcon == icon ? selectedColor : Colors.black,
                      size: 30,
                    ),
                  );
                },
